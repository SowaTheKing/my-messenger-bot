...
def train_btw_stations(src, des, date):
    url = base_url + "between/source/" + str(src) + "/dest/" + str(des) + "/date/" + str(
        date) + "/apikey/" + api_key + "/"
    print(url)
    response = requests.get(url)
    p = response.json()
    ...
    if status == 404:
        ...
        for k in p['trains']:
            text += str(k['number']) + '--' + str(k['name']) + '--' + str(
                k['src_departure_time'] + '--' + str(k['dest_arrival_time'])) + "\n"
        print(text)
        return text


def train_fare(train_no, src, des, age, date, pref, quota='GN'):
    url = base_url + "fare/train/" + str(train_no) + "/source/" + str(src) + "/dest/" + str(des) + "/age/" + str(
        age) + "/pref/" + str(pref) + "/quota/" + str(quota) + "/date/" + str(date) + "/apikey/" + api_key + "/"
    response = requests.get(url)
    p = response.json()
    status = p['response_code']
    if status == 404:
...
def seat_avail(train_no, src, des, age, date, pref, quota='GN'):
    url = base_url + "check-seat/train/" + str(train_no) + "/source/" + str(src) + "/dest/" + str(des) + "/date/" + str(
        date) + "/pref/" + str(pref) + "/quota/" + str(quota) + "/apikey/" + api_key + "/"
    print(url)
    response = requests.get(url)
    p = response.json()
    status = p['response_code']
